	...
	class BBCodeConverter {
		...
		*/
		function toXhtml($text, $bbStrict = false){
			...
			if(VERBOSE) echo "Retrieved start tags - " . count($start_tags) . " found." . PHP_EOL;
			...
			if(VERBOSE) echo "Retrieved end tags - " . count($end_tags) . " found." . PHP_EOL;
			...
			*/
			...
			$i = 1;
			$this->text = $text;
			if(VERBOSE) echo "Starting tag pair search ..." . PHP_EOL;

			while($tag_pair_count = preg_match_all('/' . $TAG_PAIR . '/xs', $text, $tag_pairs, PREG_SET_ORDER)){
				if(VERBOSE) echo "Number of pairs increasing from $num_pairs";
				$num_pairs += $tag_pair_count;
				if(VERBOSE) echo " to $num_pairs" . PHP_EOL;
		...
		* Translate attachement tags from bbCode to xhtml: <a>
                ...
                * In order to convert attachements, this class needs access to phpBB database; set the connection with
                 ...
                 * If zMigration instance is not provided, the [attachment] tag will be skpped.
		*/
		function _do_attachment($matches){
                    if(!isset($this->oMigration)) return "<file>" . $matches[4] . "</file>";
                    ...
                    if(!$attach_info) { print "EROARE LA " . $file_name; return "<file>" . $matches[4] . "</file>"; }
                    ...
                    if(strpos($attach_info->mimetype, "image") === 0){
                        return sprintf("<img src='%s' alt='%s' />", $file_info->file, $file_info->filename);
                    }
                    else
                        return sprintf("<a href='%s'>%s</a>", $file_info->file, $file_info->filename);
		}
